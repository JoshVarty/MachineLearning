...
def NextWordProbability(sampletext, word):
    separatedWords = sampletext.split(" ")
    lookup = {}
    totalNumberOfWords = 0

    for i in range(len(separatedWords) - 1):
        currentWord = separatedWords[i]
        if currentWord == word:
            nextWord = separatedWords[i + 1]
    ...
    #Normalize
    for word in lookup:
        lookup[word] = float(float(lookup[word]) / float(totalNumberOfWords))
...
print LaterWords(sample_memo,"we",1)